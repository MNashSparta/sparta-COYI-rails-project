class ResourcesController < ApplicationController
  ...
  def index
    @my_resources = Resource.where(status: 0)
  end
  ...
  def user_my_resources
    @my_resources = Resource.where(user_id: current_user.id)
  end

  # POST /my_resources
  # POST /my_resources.json
  def create
    @my_resource = Resource.new(my_resource_params)
  ...
  private
    ...
    def set_my_resource
      @my_resource = Resource.find(params[:id])
    end
